	...
	int nDelayTime = 0)
{
	...
	if (pCurl)
	{
		...
		if (!pPostField.empty())
		{
			curl_easy_setopt(pCurl, CURLOPT_POSTFIELDS, pPostField.data());
			curl_easy_setopt(pCurl, CURLOPT_POSTFIELDSIZE, pPostField.size());
		}
...
CURLcode curl_http_form_execute(
	...
	const std::string& pPostFileds = "",
	curl_proxytype nProxyType = CURLPROXY_HTTP,
	const std::string& pProxyPort = "",
	const std::string& pProxyUserPwd = "",
	bool bVerbose = false,
	int nDelayTime = 0)
{
	CURL* pCurl = 0;
	...
	if (pCurl)
	{
		...
		// Now specify the POST data
		if (!pPostFileds.empty())
		{
			//设置POST域数据
			curl_easy_setopt(pCurl, CURLOPT_POSTFIELDS, pPostFileds.data());
			curl_easy_setopt(pCurl, CURLOPT_POSTFIELDSIZE, pPostFileds.size());
		}
		if (nDelayTime > 0)
		{
			// Time-out the read operation after this amount of milliseconds
...
public:
	...
	virtual ~CurlHttpUtil() {
		CURL_HTTP_CLEANUP();
	}
public:
	static CurlHttpUtil* Inst() {
		static CurlHttpUtil curlHttpUtilInstance;
		return &curlHttpUtilInstance;
	}
};
//////////////////////////////////////////////////////////////////////

__inline static 
std::string post_data(const std::string& url, const std::string& data)
{
	std::string result = "";
	...
	curlCode = curl_http_data_execute(pbd, url.c_str(), true, pheaderdata, data);
