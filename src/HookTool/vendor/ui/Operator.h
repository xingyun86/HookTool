...
#define START_ENTRY_NOONLY __DEF_APP_DIR__ INT APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,INT nCmdShow){GDIPLUS_STARTUP();__DEF_APP_DIR_INIT__(*__argv);CSecurityProtects::Inst()->CheckValid(); int ret=__user_task(APP_DIR,__argc,__argv);GDIPLUS_CLEANUP();return(ret);}
#else
#define START_ENTRY __DEF_APP_DIR__ int main(int argc,char** argv){GDIPLUS_STARTUP();__DEF_APP_DIR_INIT__(*__argv);CHECK_MUTEX(APP_EXE);CSecurityProtects::Inst()->CheckValid();int ret=__user_task(APP_DIR,__argc,__argv);GDIPLUS_CLEANUP();return(ret);}
#define START_ENTRY_NOONLY __DEF_APP_DIR__ int main(int argc,char** argv){GDIPLUS_STARTUP();__DEF_APP_DIR_INIT__(*__argv);CSecurityProtects::Inst()->CheckValid();int ret=__user_task(APP_DIR,__argc,__argv);GDIPLUS_CLEANUP();return(ret);}
#endif

#define SHARE_ENTRY __DEF_APP_DIR__ int APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved){__DEF_APP_DIR_INIT__(*__argv);CSecurityProtects::Inst()->CheckValid();return(__user_task(APP_DIR,ul_reason_for_call));}
#else
...
#define START_ENTRY_NOONLY __DEF_APP_DIR__ int main(int argc,char** argv){__DEF_APP_DIR_INIT__(*__argv);CSecurityProtects::Inst()->CheckValid();__user_task(APP_DIR,argc,argv);CLOSE_MUTEX(APP_EXE);return 0;}

#define SHARE_ENTRY __DEF_APP_DIR__ int DllMain(const char * argp){__DEF_APP_DIR_INIT__(*__argv);CSecurityProtects::Inst()->CheckValid();__user_task(APP_DIR, 0);CLOSE_MUTEX(APP_EXE);return 0;}
#endif

#define START_ENTRY_BINARY() START_ENTRY int __user_task(const char* argp, int argc, char** argv)
#define START_ENTRY_BINARY_NOONLY() START_ENTRY_NOONLY int __user_task(const char* argp, int argc, char** argv)
